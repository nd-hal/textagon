...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Script to process text files with various options.")

    parser.add_argument('inputFileFullPath', type=str, help="Full path to the input file")
    parser.add_argument('outputFileName', type=str, help="Name of the output file")
    parser.add_argument('inputLimit', type=int, help="Limit on the number of input files")
    parser.add_argument('maxFeatures', type=int, help="Maximum number of features")
    parser.add_argument('maxNgram', type=int, help="Maximum N-gram size")
    parser.add_argument('maxCores', type=int, help="Maximum number of cores to use")
    parser.add_argument('lexiconFileFullPath', type=str, help="Full path to the lexicon file")
    parser.add_argument('vader', type=int, help="Vader setting (1 or 0)")
    parser.add_argument('wnaReturnLevel', type=int, help="WNA return level")
    parser.add_argument('buildVectors', type=str, help="Build vectors setting")
    parser.add_argument('index', type=int, help="Index setting (1 or 0)")
    parser.add_argument('removeZeroVariance', type=int, help="Remove zero variance setting (1 or 0)")
    parser.add_argument('combineFeatures', type=int, help="Combine features setting (1 or 0)")
    parser.add_argument('minDF', type=float, help="Minimum document frequency")
    parser.add_argument('removeDupColumns', type=int, help="Remove duplicate columns setting (1 or 0)")
    parser.add_argument('useSpellChecker', type=int, help="Use spell checker setting (1 or 0)")
    parser.add_argument('provideMisspellingDetailed', type=int, help="Provide misspelling details setting (1 or 0)")
    parser.add_argument('additionalCols', type=int, help="Additional columns setting (1 or 0)")
    parser.add_argument('writeRepresentations', type=int, help="Write representations setting (1 or 0)")
    parser.add_argument('exclusionsFileFullPath', type=str, help="Full path to the exclusions file")
    parser.add_argument('runType', type=str, help="Run type")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        inputFileFullPath = args.inputFileFullPath
        outputFileName = args.outputFileName
        inputLimit = args.inputLimit
        maxFeatures = args.maxFeatures
        maxNgram = args.maxNgram
        maxCores = args.maxCores
        lexiconFileFullPath = args.lexiconFileFullPath
        vader = bool(args.vader)
        wnaReturnLevel = args.wnaReturnLevel
        buildVectors = args.buildVectors
        index = bool(args.index)
        removeZeroVariance = bool(args.removeZeroVariance)
        combineFeatures = bool(args.combineFeatures)
        minDF = args.minDF if args.minDF < 1 else int(args.minDF)
        removeDupColumns = bool(args.removeDupColumns)
        useSpellChecker = bool(args.useSpellChecker)
        provideMisspellingDetailed = bool(args.provideMisspellingDetailed)
        additionalCols = bool(args.additionalCols)
        writeRepresentations = bool(args.writeRepresentations)
        exclusionsFileFullPath = args.exclusionsFileFullPath

...
def TextToFeatures (sentence, debug = False, lexicons = None, wnaReturnLevel = 5, useSpellChecker = True):
    ...
    for i, word in enumerate(collectMisspellingDetails['Word']):
        ...
        sentenceMisspelling = re.sub('(?<=[^a-zA-Z0-9])' + word + '(?![a-zA-Z0-9])', collectMisspellingDetails['Substitution'][i], sentenceMisspelling, count = 1)
    ...
    for i, word in enumerate(collectMisspellingDetails['Word']):
        ...
        MisspellingDetailed = MisspellingDetailed.replace(collectMisspellingDetails['Substitution'][i].lower(), collectMisspellingDetails['SubstitutionText'][i], 1)
    ...
    # Tokenized Sentence
    TokenizedSentence = [x[0] for x in processedText]
    ...
    # Word_Sense
    try:
        tempWS = disambiguate(' '.join(TokenizedSentence), algorithm = adapted_lesk, tokenizer = splitWS)
    ...
    # for WNAffect
    POSTreeBank = nltk.pos_tag(TokenizedSentence)
...
def VectorProcessor (data, maxNgram = 3, vader = False, maxFeatures = None, buildVectors = 'b', removeZeroVariance = True, combineFeatures = False, minDF = 5, removeDupColumns = False, classLabels = False, runLegomena = True, additionalCols = False, writeRepresentations = False, justRepresentations = False):
    ...
    #analyze = vectorizerNgram.build_analyzer()
    ...
    if writeRepresentations:
        ...
        for feature in processedFeatures:
            ...
            z.write(repFile, basename(repFile))
            os.remove(repFile)
...
'''
def RunFullProcess (lexiconpath, fullinputpath, inputLimit, outputpath, outputFileName, maxCores = False, maxNgram = 3, lexiconFileFullPath = False, vader = False, wnaReturnLevel = 5, maxFeatures = 50, buildVectors = 'b', index = False, removeZeroVariance = True, combineFeatures = False, minDF = 5, removeDupColumns = False, useSpellChecker = False, additionalCols = False, writeRepresentations = False, justRepresentations = False):

    now_time = str(datetime.now(get_localzone()).strftime(fmt))
    print('### Execution started at ' + now_time + ' ###\n')
